...
namespace DesafioAPI.Requests.Tarefas
{
    public class AdicionaNotaTarefaRequest : RequestBase
    {
        ...
        {
            jsonBody = File.ReadAllText(GeneralHelpers.ReturnProjectPath() + "Jsons/Mantis/Tarefa/AdicionaNotaTarefaJson.json", Encoding.UTF8);
        ...
        {
            jsonBody = File.ReadAllText(GeneralHelpers.ReturnProjectPath() + "Jsons/Mantis/Tarefa/AdicionaNotaComTempoTarefaJson.json", Encoding.UTF8);
            jsonBody = jsonBody.Replace("$nota", nota);
            jsonBody = jsonBody.Replace("$status", status);
        ...
        {
            jsonBody = File.ReadAllText(GeneralHelpers.ReturnProjectPath() + "Jsons/Mantis/Tarefa/AdicionaNotaComAnexoTarefaJson.json", Encoding.UTF8);
            jsonBody = jsonBody.Replace("$nota", nota);
            jsonBody = jsonBody.Replace("$status", status);
            jsonBody = jsonBody.Replace("$duracao", duracao);
